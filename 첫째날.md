...
## URL에서 ip만 추출
### referer sample ###
http://192.168.4.82/noindex/css/open-sans.css

> \"http:\/\/(?<ip>[0-9]+\.[0-9]+\.[0-9]++\.[0-9]+)\/"

index=test_index sourcetype="httpd_access" 
| rex field=_raw "\"http:\/\/(?<ip>[0-9]+\.[0-9]+\.[0-9]++\.[0-9]+)\/"
```

```
## REGEX로 user_agent 필드 중 Windows에서 접속한 로그만 추출
Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/111.0.0.0 Safari/537.36

> (?<win_log>Windows\s\w{2}\s\d+\.\d;\s\w+;\s\w+)

index=test_index sourcetype=httpd_access 
| regex user_agent="(?<win_log>Windows\s\w{2}\s\d+\.\d;\s\w+;\s\w+)"

index=test_index sourcetype=httpd_access 
| regex user_agent=".*Windows.*" | table user_agent
```
